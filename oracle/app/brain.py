"""Brain backends behind the fly-mcp oracle HTTP API.
...
whole stack can run on a laptop with no dataset.

Neither backend is a thinking being. The real one is a connectome read through
an engineered decoder: a comedy oracle, not intelligence.
"""
...
class RealFlyBrain:
    ...
    def __init__(self, stonkfly_home, data_dir):
        ...
        # decoder_threshold_hz=2, learning=True. Stonkfly's trading fields are
        # inert: no broker is ever constructed.
...
class MockFlyBrain:
    ...
    def consult(self, frame, reinforcement):
        ...
        noise = hash((seed, self.consults)) % (2**32)
        noise = (noise / (2**32)) * 2 - 1
        # Skew toward "run toward brighter things, slightly"
        right_hz = max(0.0, 1.5 + (right - 128) / 40 + noise)
        left_hz = max(0.0, 1.5 + (left - 128) / 40 + noise * 0.5)
...
class BrainService:
    """Single-brain state behind the API: lazy load, one queued reinforcement
    pulse consumed at the next consult. The lock keeps concurrent MCP calls
    from driving the same brain at once."""
